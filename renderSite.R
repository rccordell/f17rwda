...
courseTitle <- "Reading and Writing in the Digital Age"
navTitle <- "RWDA"
profName <- "Ryan Cordell"
profEmail <- "r.cordell@northeastern.edu"

# this generates a list of the HTML files created in lines 8-10
html_files <- list.files(path = ".", pattern = '*.html', full.names = FALSE)

# the code below works through the listed HTML files and generates a list of strings, formatted as required in the `_site.yml` file, that includes each of the site's pages and URLs. It generates the page name in upper case but you could tweak that as desired. This does name the pages based on their HTML filenames; if you wanted something more descriptive you'd have to write some more code here. 
...
# these collapse the list into a single string. This also renames the "Index" menu item as "Description." I put my course description in the `index.html` file so it's the first thing that loads with the site, but I think calling the page "Index" in the site menu would be confusing for students. If there were other pages you knew you'd want to rename consistently every time you generated the site, you could use the `sub` function to do so.  

pages <- paste(pages, collapse = "")
pages <- sub("Index","Description", pages)

# this pastes the standard boilerplate from the top of the `_site.yml` file together with the list of pages generated above. This also adds a final menu item with a link to email the professor established in the variables above. 
...
# finally, this writes everything generated above into (what should be) a valid `_site.yml` file. Commit the folder to github and everything should be updated!
